...
export default function ManajemenUser(){
    ...
    useEffect(() => {
        ...
        const getUserWithTotalSaldo = async () => {
            ...
            console.error("Gagal mengambil data pengguna atau jumlah saldo:", error.message);
            }
        };
       ...
       useEffect(() => {
        const getUserWithSaldoAndTransaksi = async () => {
            ...
            transaksi.forEach(({ saldo_id }) => {
                ...
                if (user_id) {
                jumlahTransaksiPerUser[user_id] = (jumlahTransaksiPerUser[user_id] || 0) + 1;
                }
            });
            ...
            console.error("Gagal mengambil data pengguna, saldo, atau transaksi:", error.message);
            }
        };
                ...
                <div className="text-[36px] font-semibold text-white leading-tight">
                      <div className="absolute top-[110px] left-1/2 transform -translate-x-1/2 w-[90%] bg-blue-50 rounded-xl shadow-lg h-[740px] z-10">                         
                        ...
                        {currentData.length === 0 ? (
                            ...
                            <tr key={item.id}>
                                <td className="px-4 py-2 border-b h-15">{indexOfFirstItem + index + 1}</td>
                                <td className="px-4 py-2 border-b">{item.nama}</td>
                                <td className="px-4 py-2 border-b">{item.email}</td>
                                <td className="px-4 py-2 border-b text-left">{item.jumlah_sumber}</td>
                                <td className="px-4 py-2 border-b text-left">{item.jumlah_transaksi || 0}</td>
